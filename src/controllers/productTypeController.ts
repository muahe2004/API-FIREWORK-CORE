...
export class ProductTypeController {
  ...
  async getProductTypeDropdown(req: Request, res: Response): Promise<void> {
    try {
      const oneDayInMilliseconds = 24 * 60 * 60 * 1000;
      const key = 'ListProductType';
      const cachedData = cache.get(key);
      if (cachedData) {
        res.json(cachedData);
      } else {
        const data = await this.productTypeService.getProductTypeDropdown();
